...
SELECT
	DISTINCT COUNT(ProductID) AS CONTEGGI_UNIVOCI
...
-- PER TABELLA REGION:
SELECT
	DISTINCT COUNT(RegionID) AS CONTEGGI_UNIVOCI
...
-- PER TABELLA SALES:
SELECT
	DISTINCT COUNT(SalesID) AS CONTEGGI_UNIVOCI
...
-- SENZA VISTA:
SELECT
	p.`ProductID` PRODOTTO
    , SUM(`ProductCost` * `Quantity`) FATTURATO_TOT
    , YEAR(`OrderDate`) ANNO
FROM
	Product AS p 
INNER JOIN 
...
-- TERZO METODO CON COUNT E VISTA:
SELECT
	COUNT(*)
FROM 
	`vw_infogenerali`
